    ...
    */
    RGraph.Rose = function (id, data)
    {
        ...
        */
		...
		this.DrawInnerCircles = function()
		{
			...
			for (var i=prop['chart.background.grid.size']; i<=this.radius; i+=prop['chart.background.grid.size'])
			{
				...
				if (i == prop['chart.radius']) co.strokeStyle = prop['chart.background.grid.color'];
				else co.strokeStyle = prop['chart.background.grid.color2'];
				
				// Hmmm... This is questionable
				co.moveTo(this.centerx + i, this.centery);
				
				// Radius must be greater than 0 for Opera to work
				co.arc(this.centerx, this.centery, i, 0, TWOPI, false);
				co.stroke();
			}
		}
        ...
        */
        this.DrawBackground = function ()
        {
			...
			co.fillStyle = prop['chart.background.grid.color3'];
			co.fill();
